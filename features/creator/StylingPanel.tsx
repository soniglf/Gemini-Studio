    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-300">
            <div className="grid grid-cols-2 gap-4">
                <VisualGridSelect label="Hair Style" value={model.hairStyle} options={OPTIONS.hairStyle.map(v=>({value:v, label:v}))} onChange={(e) => update('hairStyle', e.target.value)} />
                ...
                <VisualGridSelect label="Facial Hair" value={model.facialHair} options={OPTIONS.facialHair.map(v=>({value:v, label:v}))} onChange={(e) => update('facialHair', e.target.value)} />
            </div>
            <Input label="Fashion Aesthetic" value={model.clothingStyle} onChange={(e) => update('clothingStyle', e.target.value)} placeholder="e.g. 90s Grunge" />
