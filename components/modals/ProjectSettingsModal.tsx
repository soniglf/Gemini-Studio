...
import { X, FileText, DollarSign, Download, Upload, Trash2, Folder, Save } from 'lucide-react';
...
export const ProjectSettingsModal: React.FC = () => {
    ...
    const isOpen = activeModal === 'PROJECT_SETTINGS';
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
            <div className="w-full max-w-2xl bg-[#0B1121] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
                    <section>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Input label="Campaign Name" value={projectName} onChange={e => setProjectName(e.target.value)} />
                            <div className="flex flex-col mb-6 w-full">
                                ...
                                <div className="flex items-center gap-2 bg-slate-900/40 border border-white/10 rounded-lg px-4 py-3">
                                    ...
                                    <input className="w-full bg-transparent text-white placeholder:text-white/20 focus:outline-none" placeholder="No Limit" type="number" value={budgetInput} onChange={e => setBudgetInput(e.target.value)} />
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section>
                        ...
                        <TextArea label="Global Instructions" placeholder="e.g. Always use high contrast. Subject should look confident." value={contextText} onChange={e => setContextText(e.target.value)} className="h-32" />
                    ...
                    <section className="bg-white/5 rounded-xl p-4 border border-white/5">
                        ...
                        <div className="flex gap-3">
                            <Button variant="secondary" onClick={handleExport} className="flex-1 text-xs h-10"><Download size={14} className="mr-2"/> Export .gemini</Button>
                            <Button variant="secondary" onClick={handleImportClick} className="flex-1 text-xs h-10"><Upload size={14} className="mr-2"/> Import Project</Button>
                            <input type="file" ref={fileInputRef} className="hidden" accept=".gemini,.zip" onChange={handleImportFile}/>
                            <Button variant="danger" onClick={handleDelete} className="flex-1 text-xs h-10"><Trash2 size={14} className="mr-2"/> Delete Project</Button>
                        </div>
                    </section>
                </div>
                ...
                <div className="p-6 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3">
                    ...
                    <Button onClick={handleSave} className="px-8"><Save size={16} className="mr-2"/> Save Changes</Button>
                </div>
            </div>
        </div>
    );
