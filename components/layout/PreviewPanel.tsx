...
import { Code, Check, Copy, Tag, Plus, Send, Lock, Hash, Image as ImageIcon, Maximize2, PanelRightClose, Activity, Layout, PanelLeftOpen, ChevronLeft, ChevronRight, MonitorPlay } from 'lucide-react';
...
export const FloatingPreview: React.FC = memo(() => {
    const { lastGenerated } = useGenerationStore();
    const { assets } = useGalleryStore();
    const { togglePreviewCollapse, isGenerating } = useUIStore();
...
export const PreviewPanel: React.FC<PreviewPanelProps> = memo(({ onAssetSelect }) => {
    ...
    const { togglePreviewCollapse, addToast } = useUIStore();
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-slate-950/50 backdrop-blur-sm relative overflow-hidden group/panel border-l border-white/5">
            ...
            <button 
                onClick={togglePreviewCollapse}
                className="absolute top-4 right-4 z-50 p-2 text-white/20 hover:text-white bg-black/20 hover:bg-black/60 backdrop-blur rounded-lg transition-all"
            ...
            <div className="flex-1 relative flex flex-col items-center justify-center p-4 lg:p-8 z-20 overflow-hidden">
                {isLoading ? (
                    ...
                    <div className="text-center opacity-30 z-10 flex flex-col items-center">
